...
static bfd_vma
mips_elf_high (bfd_vma value)//ppp
{
  ...
  *cross_mode_jump_p = (!info->relocatable
  ...
  switch (r_type)
      ...
      else
	{
      ...
      printf("\b\b\b\b\b\b\b\b\b(pointer) is relocated by R_32! **\n");//ppp-
      printf("symbol %#x\t", (unsigned int)symbol );
      printf("addend %#x\t",(unsigned int)addend );
      printf(" p %#x\n",(unsigned int)p );
      printf("value = symbol + addend : %#x\t", (unsigned int)value );
      printf("dst_mask : %#x\n", (unsigned int)howto->dst_mask);//-ppp
	...
	printf("is relocated by HI16 **\n"//ppp-
"symbol %#x\tadded %#x\t p %#x\n" 
, (unsigned int)symbol, (unsigned int)addend, (unsigned int)p);//-ppp

    value = mips_elf_high (addend + symbol);
	
	printf("value = mips_elf_high (addend + symbol) : %#x\n" //ppp-
, (unsigned int)value); 
    value <<= howto->rightshift;
    printf("value<<4 : %#x\t", (unsigned int)value);
	  value &= howto->dst_mask;
    printf("dst_mask : %#x\n", (unsigned int)howto->dst_mask);//-ppp
	}
      else
	...
	printf("is relocated by LO16 **\n"//ppp-
"symbol %#x\tadded %#x\t p %#x\n" 
, (unsigned int)symbol, (unsigned int)addend, (unsigned int)p);
        value = (symbol + addend);		
        printf("value = (addend + symbol) : %#x\n" , (unsigned int)value);
        value <<= howto->rightshift;
    	printf("value<<4 : %#x\t", (unsigned int)value);
        value &= howto->dst_mask;
        printf("dst_mask : %#x\n", (unsigned int)howto->dst_mask);//-ppp
	...
	return bfd_reloc_outofrange;
	  ...
	  //value = symbol - addend - p;
      value = symbol - 4 - p;//ppp

      if (was_local_p || h->root.root.type != bfd_link_hash_undefweak)
	overflowed_p = mips_elf_overflow_p (value, 23);
      value >>= howto->rightshift;
      value <<= howto->bitpos;//ppp
      value &= howto->dst_mask;
      break;

    case R_MIPS_PC26_S2:
      if (howto->partial_inplace)
	addend = _bfd_mips_elf_sign_extend (addend, 23);//ppp 28

      if ((symbol + addend) & 3)
	return bfd_reloc_outofrange;
	  //value = symbol + addend - p;
      printf("\b\b\b\b\b\b\b\b\b(func) is relocated by PC26_S2! **\n");//ppp-	
      value = symbol + (-4) - p;
      printf("symbol %#x\t", (unsigned int)symbol );
      printf("addend %#x\t",(unsigned int)addend );
      printf(" p %#x\n",(unsigned int)p );//-ppp
	  ...
	  overflowed_p = mips_elf_overflow_p (value, 23);//ppp- 28
      printf("value = (s + (-4) - p) : %#x\n", (unsigned int)value );//-ppp
	  
      value >>= howto->rightshift;
	  
      value <<= howto->bitpos;//ppp-
      printf("value >> 2 << 5: %#x\t" , (unsigned int)value);
      printf("dst_mask : %#x\n", (unsigned int)howto->dst_mask);///-ppp
			  ...
			  bfd *input_bfd, bfd_byte *contents)
{
  ...
  unsigned int size = bfd_get_reloc_size (howto);//ppp-
  if (size != 0)	
    x = bfd_get (8 * size, input_bfd, location);//-ppp
			     ...
			     bfd_boolean cross_mode_jump_p)
{
  ...
  size = bfd_get_reloc_size (howto);//ppp-
  if (size != 0)
    bfd_put (8 * size, input_bfd, x, location);//-ppp
...
bfd_boolean _bfd_mips_elf_sort_relocs_p (asection *sec)//ppp
{
